...
import { assembleComponts, createPoleTmpl, deletePoleTmpl, calcPolePartLoad, getComponents } from '@/api/Components'
...
import { show_Form, show_ContextMenu } from '../Sun/WindowEnv.js'
...
var kcId = 715;
getComponents("standardType=0").then(res => {
	try {
		if (res.respCode == 0) {
...
function get_MicroTools(vue) {
		...
		ToolPickModule.autorun = function (manager, global, param, callevent) {
			...
			if (param.once !== undefined && param.once) {
				...
				if (this.data.allowquit)
					manager.log('log', HTML.create_TabOr([HTML.create_Button('左键') + ' 选择插槽', HTML.create_Button('右键') + ' 退出']))
				else
	...
	})
	{
		ToolPickSlot.autorun = function (manager, global, param, callevent) {
			this.data.hoversignal = param.hoversignal || false
			this.data.allowquit = param.allowquit || true
			set_TreeShowMode.call(vue, 'select_slot')
	...
	})
	{
			...
			vue.$static.Scene.baseslot.Traverse((component) => {
				...
				if (component.model !== null) {
					...
					if (component.rules.get_Module) {
						Module.set_Color(component, 0.6)
		...
		ToolPickModuleSlot.click = function (manager, global, param, callevent) {
			...
			if (ans.length === 1) {
				vue.$static.Scene.baseslot.Traverse((m) => {
					Module.set_Color(m, 1)
				})
	...
	const ToolHighlightModule = new TOOL.MicroTool('HighlightModule', '高亮')
	{
		ToolHighlightModule.autorun = function (manager, global, param, callevent) {
			...
			if (param.exclusive === true) {
				vue.$static.Scene.baseslot.Traverse((highlightobj) => {
			...
			else if (vue.$static.Scene.context.selectedModule !== null) {
				Module.set_Visible(vue.$static.Scene.context.selectedModule, true)
				let highlight_color = param.highlight_color || '#ff9800'
				Module.highlight(vue.$static.Scene.context.selectedModule, true, highlight_color)
				return { flag: 'FIN', param: { module: vue.$static.Scene.context.selectedModule } }
			}
			else {
	...
	const ToolSelectModule = new TOOL.MicroTool('SelectModule', '选中组件')
	{
		ToolSelectModule.autorun = function (manager, global, param, callevent) {
			if (param.module !== undefined) {
				...
				if (module === null) {
					if (vue.$static.Scene.context.selectedModule !== module) {
						set_SelectedModule.call(vue, module)
		...
		ToolEquipModule.autorun = function (manager, global, param, callevent) {
			if (param.module !== undefined) {
				...
				if (module instanceof Module) {
					...
					refresh_Tree(vue.$static.Scene.base)
					refresh_AllTreeData.call(vue);
					module.rules.on_Arm(module, null, null, { unit: Unit })
					set_SelectedModule.call(vue, null)
...
function create_Scene(name) {
	...
	searchDicDetailList(dicParam).then(response => {
		if (response.respCode === 0 && response.returns.length > 0) {
			dicDetailList = response.returns
		}
	})
...
export function new_Scene(item) {
	...
	if (item === null) {
		...
		currentPoleId = !filedescripter || filedescripter === null || !filedescripter.file || filedescripter.file === null ? currentPoleId : filedescripter.file.poleId
		let data = JSON.parse(filedescripter.file.data)
		console.log(data)
		try {
			ans = create_Tree_from_PoleJson(data.components, data.acrossMultiTransverseArm, scene.objectorigin, this)
			console.log(ans)
	...
	// console.log("----",this)
	// console.log(this.$static.Scene.baseslot.connectedmodule[0].get_Children()[1].get_Children()[1])
	// let imgJson = {
	// url:"group1/M00/00/12/CggUzmBmd9SEDPVAAAAAAOYGtyU4158264",
	// scale:{
	// 	x: 0.198,
	// 	y: -0.194
	// },
	...
	if (ans.equipWithSticker !== null && ans.equipWithSticker !== undefined) {
		for (let i = 0; i < ans.equipWithSticker.length; i++) {
			createImg.call(this, ans.equipWithSticker[i], ans.equipWithSticker[i].editorworkspace.sticker)
...
export function createImg(selectedModule, createJson, scene = this.$static.Scene.objectorigin) {
	debugger
	console.log(this)
	if (selectedModule !== null) {
		{
			...
			let plugin = new MP_Model('sticker', null, scene, createJson.id, createJson.roadName)
			try {
				module.add_Plugin(plugin)
				loader.load(createJson.stickerAddr, function (data) {
					...
					group.scale.y = createJson.scale.y
					group.scale.x = createJson.scale.x
					box.setFromObject(base)
					group.position.z = createJson.position.z
					group.position.y = createJson.position.y
					group.position.x = createJson.position.x
					plugin.model = base
					...
					plugin.position = { x: createJson.position.x, y: createJson.position.y, z: createJson.position.z }
					plugin.scale = { x: createJson.scale.x, y: createJson.scale.y };
					plugin.rotation = { x: createJson.rotation.x, y: createJson.rotation.y, z: createJson.rotation.z }
...
export function inspectorUpdate(action, val) {
	if (action === 'slotPosition') {
	...
	else if (action === '设计规划综合杆') {
		console.log(val)
		if (val.actionchain == '设计规划综合杆>module_select') {
			selectLibrary = val.data.module_select
			refresh_build_selectedModule_inspector.call(this, this.$static.Scene.context.selectedModule)
		}
		if (val.actionchain !== '设计规划综合杆>upload') return
		if (currentPoleId === '') {
			alert("合杆Id不能为空")
			return
		}
		...
		switch (val) {
			case '指示牌标志贴图': {
				let that = this
				const loader = new SVGLoader()
				let plugin = new MP_Model('sticker', null, this.$static.Scene.objectorigin)
	...
	if (this.$static.Scene.reactive.selectedMode === 0) {
		...
		this.$static.Scene.context.checkLoadResult.forEach((item) => {
			...
			if (module === null) {
				...
				list.push({ type: 'checkload', title: module.name + "弯矩", action: 'select_Module', itemvalue: { value: item.data.currBend, max: module.property.ratedBend === null ? '未知' : parseFloat(module.property.ratedBend), uid: item.uid, valid: item.data.vaildFlag, selected: selected === null ? false : item.uid === selected.uid } })
				if (item.data.currTorque !== undefined) {
					list.push({ type: 'checkload', title: module.name + "扭矩", action: 'select_Module', itemvalue: { value: item.data.currTorque, max: module.property.currTorque === null ? '未知' : parseFloat(module.property.ratedTorque), uid: item.uid, valid: item.data.vaildFlag, selected: selected === null ? false : item.uid === selected.uid } })
				}
			}
		})
		this.$EventBus.$emit('inspectorInit', {
	...
	if (createJson.classification === '贴图') {
		if (this.$static.Scene.context.selectedModule !== null) {
			console.log(createJson)
			{
				...
				let that = this
				const loader = new SVGLoader()
				let plugin = new MP_Model('sticker', null, this.$static.Scene.objectorigin, createJson.id)
				try {
					module.add_Plugin(plugin)
...
function check_Tree() {
	...
	let mainpoleModule = null
	{
		...
		let others = mainpolearray.filter((module) => { return module.classification !== '主杆' && module.classification !== '微型杆' })
		if (others.length > 0) {
...
function save(poleid, polecode) {
	...
	// 临时代码
	object.components.mainPole.poleType = 3
	let word = JSON.stringify(object)
	assembleComponts(object).then((res) => {
		// //console.log(res)
		if (res.respCode === 0) {
			customLog(this, 'log', 'upload', '编辑器 提示', HTML.create_KeyPair('操作', '设计规划综合杆', 'String') + ' 完成')
		}
		else {
			customLog(that, "error", "upload", "设计规划综合杆 错误", "在 <请求数据> 出现了如下错误: <br>" + HTML.create_KeyPair('错误', res.respMsg, "String"))
		}
	})
		.catch(error => {
			customLog(that, "error", "upload", "设计规划综合杆 错误", "在 <请求数据> 出现了如下错误: <br>" + HTML.create_KeyPair('错误', error.res.respMsg, "String"))
		})
	// }
	// })
}
...
export function checkLoad() {
	...
	calcPolePartLoad(param).then(res => {
		console.log(res)
		if (res.respCode === 0) {
			set_checkLoad_Result.call(this, res.returns)
			alert("请求成功")
		}
		else {
			customLog(that, "error", "荷载分析", "网络接口生成 错误", "在 <请求数据> 出现了如下错误: <br>" + HTML.create_KeyPair('错误', res.respMsg, "String"))
		}
	})
}
...
function get_SaveObject(poleid, polecode) {
	...
	// 主杆
	{
		...
		let otherarray = []
		children.forEach((module) => {
			if (module.classification === '主杆' || module.classification === '微型杆') mainpolesarray.push(module)
			else otherarray.push(module)
		})
		if (otherarray.length > 0) {
			let otherstr = otherarray.map((module) => { return HTML.create_KeyPair('ComponentPath', module.get_Path(), 'String') })
		...
		if (mainpolesarray.length === 0) {
			...
			param.mainPole = {
				...
				poleArms: [],
				connectors: [],
				poleType: parseInt(mainpole.property.poleType)
			}
			paramtree.mainPole = {
				module: mainpole,
				vicePole: null,
	...
	if (paramtree.mainPole !== null) {
				...
				connectorsarray.forEach((connector, index) => {
					...
					// }
						...
						equiparray.forEach((equip) => {
							let equipposition = equip.get_WorldPosition_HSL()
							let property = equip.get_Property()
							if (property.equipId != undefined) {
								property.equipCode = property.equipId
								delete property.equipId
							}
							let ans = {
								// equipId: 0,
								componentId: parseInt(equip.componentid),
							...
							console.log(equip)
							if (equip.editorproperty.sticker !== null && equip.editorproperty.sticker !== undefined) {
								ans.sticker = {
									id: equip.editorproperty.sticker.id,
									scale: {
										x: equip.editorproperty.sticker.scale.x,
										y: equip.editorproperty.sticker.scale.y,
									},
									rotation: {
										x: equip.editorproperty.sticker.rotation.x,
										y: equip.editorproperty.sticker.rotation.y,
										z: equip.editorproperty.sticker.rotation.z,
									},
									position: {
										x: equip.editorproperty.sticker.position.x,
										y: equip.editorproperty.sticker.position.y,
										z: equip.editorproperty.sticker.position.z,
									},
									roadName: equip.editorproperty.sticker.text,
								}
							}
							property.orgType = property.orgType && property.orgType.selectitem ? property.orgType.selectitem.code : null
							Object.assign(ans, property)
							param.mainPole.vicePole.connectors[index].carryEquips.push(ans)
							paramtree.mainPole.vicePole.connectors[index].carryEquips.push({
								module: equip
				...
				if (equiparray.length === 0);
					...
					equiparray.forEach((equip) => {
						...
						let ans = {
							...
							lookingDirectionAngle: equip.get_WorldRotation().y,
							elevationAngle: equip.get_WorldRotation().x,
							equipWidth: property.equipWidth,
							equipLength: property.equipLength,
							xaxis: equipposition.x,
							yaxis: equipposition.y,
							zaxis: equipposition.angle
						}
						...
						// 判断是2F还是3F
						// dicDetailList.forEach((item)=>{
						// 	let floor = "2"
						// 	if(parseInt(item.dicValue) === equip.componentid && item.typeId === 282){
						// 		if(item.dicCode === 3){
						// 			floor = "3"
						// 		}
						// 		if( typeof acrossMultiRecordUidBean[floor] === 'undefined' || acrossMultiRecordUidBean[floor] === null){
						// 			item.dicCode === 2 ? acrossMultiRecordUidBean = { "2": []} : acrossMultiRecordUidBean = {"3" : []}
						// 		}
						// 		if(typeof acrossMultiRecordUidBean[floor].maxTransverseArmUid === 'undefined' ||
						// 			acrossMultiRecordUidBean[floor].maxTransverseArmUid === null ||
						// 			arm.yaxis > maxArmHeight){
						// 			maxArmHeight = arm.yaxis
						// 			acrossMultiRecordUidBean[floor] = {
						// 				"equipmentUid": parseInt(equip.uid),
						// 				"maxTransverseArmUid": parseInt(arm.uid)
						// 			}
						// 		}
						// 	}
						// })
		...
		if (equiparray.length === 0);
			...
			equiparray.forEach((connector, index) => {
				...
				if (connector.editorproperty.sticker !== null && connector.editorproperty.sticker !== undefined) {
					ans.sticker = {
						id: connector.editorproperty.sticker.id,
						scale: {
							x: connector.editorproperty.sticker.scale.x,
							y: connector.editorproperty.sticker.scale.y,
						},
						rotation: {
							x: connector.editorproperty.sticker.rotation.x,
							y: connector.editorproperty.sticker.rotation.y,
							z: connector.editorproperty.sticker.rotation.z,
						},
						position: {
							x: connector.editorproperty.sticker.position.x,
							y: connector.editorproperty.sticker.position.y,
							z: connector.editorproperty.sticker.position.z,
						},
						roadName: connector.editorproperty.sticker.text,
					}
				}
				property.orgType = property.orgType && property.orgType.selectitem ? property.orgType.selectitem.code : null
				Object.assign(ans, property)
	...
	crossModules.forEach((equip) => {
		let parents = [equip.get_Parent()].concat(equip.linkslotlist.map((slot) => { return slot.belong }))
		let connectparent = parents.reduce((ans, parent) => {
			let height = ans.get_WorldPosition_HSL().y
			if (height > parent.get_WorldPosition_HSL().y) return ans
			else return parent
		}, parents[0])
		let count = parents.length
		if (acrossMultiRecordUidBean[count] === undefined) {
...
function uploadTemplate(json, id = null, that) {
	...
	if (id && id !== "") {
		word2 = 'templateId=' + id + "&standardType=" + json.standardType
	} else {
		word2 = "standardType=" + json.standardType
	}

	// if (json.standardType === 1) {
	// 	word2 += "&orgCompanyId=" + that.listQuery.orgCompanyId
	// }

	delete json.standardType
	createPoleTmpl(word2, json).then((res) => {
		//console.log(res)
		if (res.respCode === 0) {
			customLog(this, 'log', 'upload', '编辑器 提示', HTML.create_KeyPair('操作', '设计综合杆模板', 'String') + ' 完成')
		} else {
			customLog(that, "error", "upload", "设计综合杆模板 错误", "在 <请求数据> 出现了如下错误: <br>" + HTML.create_KeyPair('错误', res.respMsg, "String"))
		}
	})
}
...
export async function export_DXF() {
	//console.log(">> export_DXF")
...
export function set_ToolMode(mode) {
	...
	if (mode === 0) {
		...
		this.$static.Scene.reactive.additionaldistance = 0;
		update_Additional.call(this)
		let selected = this.$static.Scene.context.selectedModule;
		if (selected !== null) {
			update_TransformHelper.call(this, this.$static.Scene.context.selectedModule);
			if ([0, 1, 2, 7].includes(selected.classificationid)) {
				this.$static.Scene.transformcontrol.visible = false;
				this.$static.Scene.transformcontrol.enabled = false;
				this.$static.Scene.transform.origin.position.set(100000, 100000, 100000)
				this.$static.Scene.transform.origin.rotation.set(100000, 100000, 100000)
		...
		// this.$EventBus.$emit('app_open_Popup', 'pw-checkLoad', 'app', '荷载计算', 300, 400, true, true, true, {})
...
export function update_Additional() {
	this.$static.Scene.base.Update(undefined, undefined, this.$static.Scene.reactive.additionaldistance)
}
/*
...
function GetBox(module) {
	if (module.classification == "主杆" && module.name == "主杆" || module.classification === '微型杆') {
		let box = new THREE.Box3();
...
function MainView() {
	...
	for (let i = 0; i < moduleAndSlot.length; i++) {
		...
		//console.log(moduleAndSlot[i].classification + '包围盒')
		//console.log(moduleAndSlot[i].box)
		if (moduleAndSlot[i].classification == "横臂") {
			indexOfVice.push(i);
		}
		if (moduleAndSlot[i].classification == "主杆" || module.classification === '微型杆') {
	...
	for (let i = 0; i < indexOfConnector.length; i++) {
		if (i == 0 && indexOfConnector[0].length != 0) {
			if (indexOfConnector[i].length == 1) {
				...
				let points = [];
				console.log("enter")
				console.log(moduleAndSlot[indexOfConnector[i][0]])
				...
				DrawLine.call(this, points, Math.abs(0 - moduleAndSlot[indexOfConnector[i][0]].world_position.y) * 100, auxiliaryLinePoints1, auxiliaryLinePoints2);
			} else {
...
function TopView() {
	...
	for (let i = 0; i < moduleAndSlot.length; i++) {
		...
		} else if (moduleAndSlot[i].classification === "横臂") {
			let level = 1
			let points = [];
			let auxiliaryLinePoints1 = [];
			let auxiliaryLinePoints2 = [];
...
async function export2PDF() {
	...
	imgData.push(this.$static.Renderer.domElement.toDataURL("image/jpeg"));//这里可以选择png格式jpeg格式
	...
	let promise = new Promise(function (resolve, reject) {
		calcPolePartLoad(param).then(res => {
			...
			if (res.respCode === 0) {
				...
				let array = set_checkLoad_Result.call(that, res.returns)
...
// 	try {
// 		let path = await show_Form('pf-file-system-dialogue', { exclusive: false }, { path: FileSystem.ROOT.name, savetype: 'pole', savename: '合杆' }, "测试")
// 		console.log(path);
// 	}
// 	catch (err) {
// 		console.error(err);
...
export async function diff() {
	this.$refs['pole_实验性'].show = false;
	let a = this.$static.Scene
	let b = this.$parent.$refs['Canvas2'].$static.Scene
	if (a === null || b === null) {
		this.$EventBus.$emit('console_add_Output', 'error', '无法比较', HTML.create_KeyPair('操作', 'Diff SceneTree', 'String') + ' 无可比较项目')
		return;
	}
	a = a.baseslot.connectedmodule[0]
	b = b.baseslot.connectedmodule[0]
		...
		function cmp(roota, rootb, layer = -1) {
			if (roota.root !== true && rootb.root !== true) {
				if (roota.worldPositon[0] === rootb.worldPositon[0]
					...
					res.push({ type: 'same', uid: rootb.uid, path: rootb.module.get_Path(), str: getLayer(layer) + Module.get_StyledHTML(roota.module) })
				}
				else {
					Module.set_Color(roota.module, 1.0, '#0000ff')
					// Module.set_Color(rootb.module, 1.0, '#0000ff')
					res.push({ type: 'chg', uid: roota.uid, path: roota.module.get_Path(), str: getLayer(layer) + HTML.create_Strong("CHG", "rgb(230 128 20)") + " " + Module.get_StyledHTML(roota.module) + " " + HTML.create_KeyPair("位置", `${Math.round(roota.worldPositon[0] / Unit)}, ${Math.round(roota.worldPositon[1] / Unit)}, ${Math.round(roota.worldPositon[2] / Unit)} -> ${Math.round(rootb.worldPositon[0] / Unit)}, ${Math.round(rootb.worldPositon[1] / Unit)}, ${Math.round(rootb.worldPositon[2] / Unit)}`, "String") + " " + HTML.create_KeyPair("旋转", `${roota.worldRotation[0]},${roota.worldRotation[1]},${roota.worldRotation[2]} -> ${rootb.worldRotation[0]},${rootb.worldRotation[1]},${rootb.worldRotation[2]}`, "String") })
				}
			}
			...
			roota.subs.filter((a) => {
				...
				a.module.Traverse((m, l) => {
					Module.set_Color(m, 1.0, '#ff0000')
					res.push({ type: 'del', uid: m.uid, path: m.get_Path(), str: getLayer(l + layer + 1) + " " + HTML.create_Strong("DEL", "red") + " " + Module.get_StyledHTML(m) })
				})
			}))
			samesubs.forEach((i) => {
				cmp(i[0], i[1], layer + 1)
			...
			rootb.subs.filter((a) => {
				...
				a.module.Traverse((m, l) => {
					Module.set_Color(m, 1.0, '#00ff00')
					res.push({ type: 'add', uid: m.uid, path: m.get_Path(), str: getLayer(l + layer + 1) + " " + HTML.create_Strong("ADD", "rgb(29 155 34)") + " " + Module.get_StyledHTML(m) })
				})
			}))
	...
	let ans = _diff(b, a)
	try {
		await show_Form('pf-diff-inspect', { exclusive: false }, { data: ans }, "Diff Inspect")
	}
	catch (err) {
