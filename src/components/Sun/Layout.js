///////////////////////////////////////////////////////////////
//          SUN Layouter a panel management Library
///////////////////////////////////////////////////////////////

// class Panel manage a panel
// - main panel / control panel / switch panel
// - main panel to be show in window - new Panel(unique_name_id, nickname, resize_callback, expand_x, expand_y, min_width, min_height, visible, switch_callback)
// - resize_callback => (x, y, width, height, visible)=>{}
// - switch_callback => (list, width, height)=>{}
export class Panel {
	...
	refresh_Panel(x, y, width, height, visible = true) {
		visible = visible ? this.visible : false
		if (this.panel_object !== null) {
			...
			this.panel_object.style.pointerEvents = visible ? 'unset' : 'none'
	...
	// create a list this can only be called on a control panel
	...
	get_Visible() {
		...
		if (this.panel_object !== null) {
			...
			this.panel_object.style.pointerEvents = 'none'
			return this.visible
		}
		else if (this.is_ControlPanel() && !this.control_switch) {
			let visible = false
			for (let i = 0; i < this.children.length; i++) {
				let childvisible = this.children[i].get_Visible()
	...
	// add a main panel after this main panel
	// - direction = true horizatial
	...
	// add a main panel before this main panel
	// - direction = true horizatial
	// - direction = false vertical
	...
	// switch to *name* in a switch panel
	switch(name) {
	...
	set_Size(width = null, height = null) {
		width = width === null ? null : parseFloat(width)
		height = height === null ? null : parseFloat(height)

		if (this.is_ControlPanel()) {
			return
		}

		else if (!this.parent.control_switch) {
			if (this.parent.direction) {  // H
				if (height !== null) {
					this.parent.children.forEach((panel) => {
						if (!panel.is_ControlPanel()) {
							panel.min_height = height
						}
					})
				}
				if (width !== null)
	...
	get_Info(layer = 0) {
		...
		if (this.is_ControlPanel()) {
			return summary + "+ Control " + this.name + ' ' + (this.direction ? "H" : "V") + (this.control_switch ? ' Switch ' + this.current_panelname : '') + '(' + this.visible + ', ' + this.min_width + ", " + this.min_height + ")" + this.panel_object
		}
		else {
			return summary + "  Panel " + this.panel_id + ' Name: ' + this.name + ' (' + this.visible + ', ' + this.min_width + ", " + this.min_height + ")" + this.panel_object
		}
	}
	...
	get_SaveObj() {
		...
		if (this.is_ControlPanel()) {
			ans = {
				...
				expand_x: this.expand_x,
				expand_y: this.expand_y,
				visible: this.visible,
				control_switch: this.control_switch,
				current_panelname: this.current_panelname
			}
		}
		else {
			ans = {
		...
		ans.children = []
		this.children.forEach((panel) => {
			ans.children.push(panel.get_SaveObj())
		})
		return ans
	}
}
...
// class Layouter
// - new Layouter(layouter_name_id : string, [list of main panel's *unique_name_id*] : Array<string>, dragbox : DOMElement = null, margin : float = 2, save : bool)
// - use layouter.layout_tree to access the root panel
export class Layouter {
	constructor(layout_id, all_panel_id, dragbox = null, margin = 2, save = false) {
		this.layout_id = layout_id
		this.layout_main = document.getElementById(this.layout_id)
		this.visible = true
		this.layout_tree = new Panel(null, 'root')
		...
		if (dragbox !== null) {
			this.panel_id_list.forEach((panelid) => {
				...
				if (object !== null) {
					this.dragover_func = (e) => { this.on_Tab_over(e, panelid) }
					this.dragleave_func = (e) => { this.on_Tab_leave(e) }
					this.drop_func = (e) => { this.on_Tab_drop(e, panelid) }
					object.addEventListener('dragover', this.dragover_func)
					object.addEventListener('dragleave', this.dragleave_func)
					object.addEventListener('drop', this.drop_func)
				}
	...
	// need to be called before web page refresh or vue app before destroy
	release_Event() {
		if (dragbox !== null) {
			this.panel_id_list.forEach((panelid) => {
				// //console.log(panelid)
				let object = document.getElementById(panelid)
				if (object !== null) {
					object.removeEventListener('dragover', this.dragover_func)
					object.removeEventListener('dragleave', this.dragleave_func)
					object.removeEventListener('drop', this.drop_func)
				}
	...
	on_Tab_drop(event, panelid) {
		// //console.log(event)
		this.drag_box.style.visibility = 'hidden'
		let panelidfrom = event.dataTransfer.getData("PanelDragData")
		...
		if (this.save !== undefined)
			window.localStorage[this.save] = JSON.stringify(this.save_Layout())
		event.preventDefault()
	}
	...
	// you need to call this after you changed layout tree's structure
	format() {
		this.layout_tree.format()
		this.layout_tree.update_Parent(null)
	}

	// set the whole layout's visibility
	set_Visible(visible) {
		this.visible = visible
	}

	// refresh layout , remember to call format() before calling this
	refresh_Layout() {
	...
	// get a main panel by its unique_name_id
	...
	// print the layout tree, useful when debugging
	print_Layout() {
		let panel_stack = new Array()
	...
	// - return obj
	save_Layout() {
		return this.layout_tree.get_SaveObj()
	}

	create_Panel(panelobj, panellist) {
		let panel
		if (panelobj.type === "control") {
			panel = new Panel(panelobj.panel_id, panelobj.name, null, panelobj.expand_x, panelobj.expand_y, panelobj.min_width, panelobj.min_height, panelobj.visible, null)
			panel.control_switch = panelobj.control_switch
			panel.direction = panelobj.direction
			panel.current_panelname = panelobj.current_panelname
			panel.children = panelobj.children.map((panelobj2) => {
				return this.create_Panel(panelobj2, panellist)
			})
		}
		else {
			panel = panellist[panelobj.panel_id]
			panel.min_width = panelobj.min_width
			panel.min_height = panelobj.min_height
			panel.expand_x = panelobj.expand_x
			panel.visible = panelobj.visible
			panel.expand_y = panelobj.expand_y
		}
		return panel
	}

	// load layout from saved obj
	load_Layout(saveobj) {
		let panellist = {}
		this.get_AllPanel().forEach((panel) => {
			panellist[panel.panel_id] = panel
		})
		this.layout_tree = this.create_Panel(saveobj, panellist)
		this.format()
		this.refresh_Layout()
	}
}
