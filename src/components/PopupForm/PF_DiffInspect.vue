				...
				<div class="v-box">
					<div class="topbar" style="overflow: visible;">
						<div class="tab selected">
							...
							<a class="tab-text" id="PopupTitle">Diff Result </a>
						</div>
							...
							<div class="v-box">
								<div class="scroll-container flex">
									<div class="listview-dark">
										...
										<div class="form-vcontainer right">
											...
											<div class="form-container left gap">
												<div class="button-dark" @click="cut = !cut">
									...
									<div class="scrollview-dark flex">
										<div class="form-vcontainer left" style="padding: 4px 10px; gap: 10px;">
											<template v-for="item in get_List">
												<div v-html="item" style="word-break: keep-all;min-width: max-content;">
												</div>
											</template>
										</div>
	...
	export default {
		...
		computed: {
			get_List() {
				if (!this.cut) {
					let newlist = [];
					this.Input.data.filter(i => i.type !== 'same').forEach(e => {
						newlist.push(`<div class="select-dark">${e.path}</div>`)
						newlist.push(e.str)
					});
					return newlist
				}
				else
					return this.Input.data.map(i => i.str)
			}
		},
		watch: {
		},
		...
		mounted() {
			...
			this.Popup.resize(this.PopupParam.width === undefined ? 600 : this.PopupParam.width, this.PopupParam.height == undefined ? 600 : this.PopupParam.height)
