<template>
	<div class="cover" ref="PopupCover">
		<div class="Layout-Panel" ref="PopupPanel">
			<div class="container"
				...
				<div class="v-box">
					<div class="topbar" style="overflow: visible;">
						<div class="tab selected">
							<svg class="tab-icon-svg" viewBox="0 0 1024 1024" version="1.1"
							...
							<a class="tab-text" id="PopupTitle">文件管理</a>
						</div>
								...
								<div style="display: flex; flex-direction: column; flex: 1; padding: 6px; gap: 6px;">
									...
									<div class="listview-dark">
										<div class="form-container left gap">
											<div class="button-dark" :class="{'selected': showTree}"
											...
											<div class="form-container hbuttonlist">
												<div class="button-dark" :class="{'inactive': !can_GoBack}"
													@click="go_Back()">
													<svg class="button-icon-svg" viewBox="-100 -100 1224 1224"
										...
										<g transform="translate(-561 -534)">
											...
											<div class="form-container hbuttonlist left" style="flex: 1;">
												<div v-for="path, idx in get_PathList" class="button-dark"
													:class="{'selected': idx === get_PathList.length-1}"
									...
									<div style="display: flex; flex-direction: row; flex: 1; gap: 6px; height: 100px;">
										...
										<div
											style="width: 100%; height: 100%; display: flex; flex-direction: column; gap:6px">
											<div class="itemview-dark flex" @dblclick="renamePath = ''"
												...
												<div v-if="can_GoBack" class="item" style="min-width: 110px;"
													@dblclick="renamePath === '' && go_Back()" @mousedown.stop.right
													@dragover.prevent @drop="drop_on_Parent()">
										...
										{{item}}
													...
													<div v-if="item.type === 'folder'" class="item"
														...
														@mousedown.stop.right="rightclick_Item(item)"
														:draggable="renamePath === ''" @dragstart="drag(item)"
													...
													<div v-else-if="item.type === 'file'" class="item"
														style="min-width: 110px;" @dblclick="renamePath = ''"
														@mousedown.stop.right="rightclick_Item(item)"
														:draggable="renamePath === ''" @dragstart="drag(item)"
														@dragover.prevent @drop="drop(item)">
														<div class="form-vcontainer center">
									...
									<div class="listview-dark">
										<div class="form-container right gap">
											<template>
												<input class="lineedit-dark" style="flex: 1;" placeholder="输入文件名"
													v-model="saveFileName"
													:style="{'background-color': can_Save? 'var(--ObjectColor)': 'var(--ElementColorRed)'}" />
												<div class="button-dark"
													style="background-color: var(--ObjectBGColor);">
											...
											<div class="button-dark" :class="{'inactive': !can_Save}"
												@click="can_Save && confirm()">
...
<script>
	...
	import * as WindowEnv from '../Sun/WindowEnv.js'
	...
	export default {
		...
		methods: {
			...
			confirm() {
				this.Handler.resolve(this.selectedPath + '/' + this.saveFileName + '.' + this.Input.savetype);
				this.end();
			},
			rename(item) {
			...
			async close() {
				try {
					let ans = await WindowEnv.show_Form("pf-confirm", undefined, { title: "关闭保存弹窗", description: "是否要取消保存？" }, 'filesytemdialogue before close', this.Self)
					if (ans) {
						this.end()
					}
				}
		...
		async mounted() {
			...
			this.Popup.set_Resizeable(this.PopupParam.resize === undefined ? true : this.PopupParam.resize)
			this.Popup.resize(this.PopupParam.width === undefined ? 600 : this.PopupParam.width, this.PopupParam.height == undefined ? 500 : this.PopupParam.height)
			this.Popup.center(this.PopupParam.center === undefined ? true : this.PopupParam.center)
			this.Popup.popup(true, this.PopupParam.exclusive === undefined ? true : this.PopupParam.exclusive);
